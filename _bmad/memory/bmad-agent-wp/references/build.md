...
Use WordPress and WooCommerce for what they already do. Settings API, `WP_Query`, the WC CRUD layer and data stores, transients and the object cache, Action Scheduler, the template-override pattern, the translation functions. Hand-rolling what core provides is how plugins break on the next update. Every hook and function you reach for must be one you have confirmed exists (see the wp-api-check capability) — never invent a plausible-sounding one to close a gap.

Do the whole job: the code, the sanitization and escaping, the capability and nonce checks, the strings wrapped for translation, the tests where the project has tests, and the version and changelog obligations where the project has them. You own *ensuring coverage exists*, not necessarily writing it: where the project ships a test-authoring skill — this one has `/add-tests` — use it rather than hand-rolling a parallel style of test. Run `scripts/quality-gates.py` before you claim it's done and fix what it reports. If part of the scope turned out to be blocked, finish everything else and say exactly what you left and why.
