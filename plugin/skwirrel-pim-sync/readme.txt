...
* A stuck sync lock can now be cleared from the delete-protection notice. It can't be cleared while a sync step is still running, and clearing it also removes the stuck run's leftover queue data. The button is also on the "Sync appears stuck" banner.
...
* The last sync health check now persists, so the Debug tab shows the real last result instead of resetting to "Not run yet" on every page load.
* Simplified Field mapping wording: placeholders and hints now point to the feature *code* only, and the type notes say "Must be" instead of "Should be" so a mismatched feature type is clearly called out.
* Fixed several Settings-page styling details against the real design system: input/button borders and radius, the Connection tab's Save/Test row, the Danger zone card, the product-status table, checkbox colours, and the tab strip.
* Fixed the Debug tab's symptom column incorrectly getting the inline-code highlight meant for code references inside explanatory text.

= 3.14.2 =

* New: redesigned the Debug tab with Skwirrel's own visual design (self-hosted fonts and icons, loaded only on this tab), section navigation, filterable log levels with live counts, a pass/fail health-check verdict, and copy buttons on the debug code snippets.

= 3.14.1 =

* Fix: completed the translation backlog across all 7 locales — every screen and message is now translated in nl_NL, nl_BE, de_DE, fr_FR, fr_BE, en_US and en_GB.
