...
function skw_field_mapping_strings(): array
{
    return [
        ...
        'Field mappings read product-level custom classes. Setting a custom class collection ID under "What to sync" narrows which collection they are read from; without one, every collection is searched. Clearing a field turns its mapping off again, and the next synchronisation then returns that field to its default value.',
        'Stock quantity',
        'Product title',
        'Short description',
        'Long description',
        'e.g. STOCK_QTY',
        'e.g. PRODUCT_TITLE',
        'Must be a numeric feature.',
        'Must be a short text feature.',
        'Must be a long text feature.',
        'The code of one product-level custom feature holding the stock quantity. Only a whole number is used; range, logical and multi-value features are ignored. A product with no usable value keeps its current stock untouched. Trade-item level features are not used, and products priced on request keep their own availability regardless.',
        'The code of the custom feature holding the product title. Leave empty to keep using the normal source (the ERP description, then the product translations). When a product has no value for this feature, the normal source is used for that product — the title is never blanked.',
        'The code of the custom feature holding the short description. Leave empty to keep using the product translations. A product without a value keeps the normal source.',
        'The code of the custom feature holding the long description. Leave empty to keep using the normal source. Formatting is kept; unsafe markup is removed. A product without a value keeps the normal source.',
    ];
}
