...
* **Custom class collection ID is no longer a required setting.** Enabling custom classes, trade-item custom classes, or grouped products used to mark it required and block saving without it. It now saves freely; the actual safeguard remains in `Sync_Service::run_sync()`, which still fails a run immediately with a clear message if a feature that needs the collection ID is on and it is missing — that requirement is a fact about the sync, not about the settings screen.
