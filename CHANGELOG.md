...
* The connection test call stays deliberately unfiltered: `collection_ids`, `dynamic_selection_id` and the new Context ID are **not** applied to the reported product total. Filtering the count — which is what would catch "connection fine, zero products sync" — is a recorded follow-up, not part of this release. (story 5.4)
