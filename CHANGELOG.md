...
* **Clear a stuck sync lock from the delete-protection notice.** When the delete-lock is only held by a stale run state, the notice says so and offers a "Clear stuck sync lock" button, backed by an AJAX handler that re-checks the stuck state server-side before clearing. A stale-looking run is not necessarily dead — one step's API calls (120s timeout plus retries) can outlast the stuck threshold — so each step now holds a MySQL advisory step lock, and the release refuses while a worker holds it. Releasing a run also removes its queue rows, group map and membership sweep, so an abandoned run no longer leaves them behind. The synchronous (manual loopback) driver holds the same lock for its whole run, and the lock names are namespaced per installation (database name + table prefix) so two shops on one MySQL server never contend for them. The button is also offered on the "Sync appears stuck" banner, which — unlike the delete-lock notice — keeps showing once the run state is older than the 15-minute delete-lock window. Covered by `tests/Unit/ForceClearStuckRunTest.php`.
...
* **Confirm dialogs showed a literal `\n\n`.** The purge, reset-settings and delete-protection confirm messages were single-quoted PHP strings, so `\n` was a backslash-n that `wp_json_encode()` / `wp_localize_script()` passed through as text. They are double-quoted now (real line breaks) and every locale's translation follows.
* **Hardcoded English error texts are translatable.** The log viewer displayed raw AJAX errors ("Log file not found", "Could not open log file", "Invalid filename", "Access denied"); these and the remaining `wp_send_json_error()` / `wp_die()` texts now go through gettext. `TranslationCompletenessTest` fails on any new hardcoded response text, any single-quoted `\n` in a translatable string, and any POT msgid missing from a locale.
* **Translation review across all seven locales.** Fixed translations that dropped half their meaning (e.g. "Live sync log and variation attribute debugging."), wrong meanings (German "View all" / "Sync Options" / "Sync Logs", "subdomain" for "address" in de/fr/nl), labels left in English in Dutch ("Batch size", "Sync interval", "Custom class filter", …), grammar, and inconsistent status names; en_GB/en_US now fill the plural Deprecated label. Obsolete `#~` entries were dropped.
* **A non-scalar Context ID submission could reset the sync context.** A crafted `context_id[]=7` skipped the sanitiser's assignment, stored `null` as the effective context and armed a full sync against the Skwirrel default. It is now rejected like any other invalid value and the configured context stays in use.
* **`sync_etim` and `sync_manufacturers` are read from the run's frozen settings** when mapping attributes, so saving settings mid-run no longer splits one run's products across two attribute configurations.
...
* **Debug page header and section nav** now match the design: breadcrumb link in primary blue, header actions aligned with the description, and pill jump-links with an active state that follows the URL fragment, so the Debug submenu's deep link to the health check and back/forward navigation highlight the right section.
...
* **Forty-odd settings now sit in tabs — Connection, What to sync, Advanced, and Field mapping once Epic 6 added it (story 5.1).** The eight field groups were previously one long scroll; finding a setting meant scrolling past the ones you were not looking for. No field was added, removed or re-ordered within its group. Media & Language now sits under What to sync, and Permalinks under Advanced.
* **The save request is unchanged (story 5.1).** Every panel stays in the DOM, inside the one `options.php` form, never disabled — inactive ones are only hidden. `sanitize_settings()` therefore still receives the identical `$input` array. This matters because it validates *across* groups: `super_category_id` is required when category sync is on, and that pair now sits on different tabs. A per-tab save would fire that rule against missing input and wipe stored values, and every absent checkbox would read as "off".
...
* **The Context ID field is hidden on the settings screen.** Almost no shop needs it, so it no longer shows next to Timeout and Retries. It is only hidden — the input, its stored value and every read/sanitisation/API path behind it are untouched, so an install that already has a context configured keeps using it exactly as before.
* **The Field mapping tab now says which feature type each field expects.** Stock quantity, Product title, Short description and Long description each open with a bold one-line requirement — numeric for stock, short text for the title, long text for both descriptions — ahead of the existing explanation, so picking a Skwirrel feature that will not work is visible before saving instead of only showing up as a value that is silently ignored.
* **Danger zone is now its own settings tab, not a section under every tab.** "Delete all Skwirrel products" and "Reset Skwirrel sync settings" used to render below the settings form regardless of which tab was open. They now live behind a dedicated "Danger zone" tab, reachable from the tab strip or the dashboard's existing Danger Zone quick link. Both actions are unchanged — same confirmation checkbox, same nonces, same admin-post.php handlers — only where they show up moved.
...
* **Custom classes are fetched whenever a mapping needs them (stories 6.1–6.3).** A configured mapping requests `include_custom_classes` even when both custom-class *sync* toggles are off, and the attribute ID whitelist is not applied to that request — a mapped feature can belong to a class outside the whitelist, and the mapping has no class selector of its own. Local filtering of exported attributes is unchanged. A mapping resolves whether or not a custom class collection ID is configured — see the Custom class collection ID entry above.
* **Field mapping now links to where a feature's ID or code is found.** Same style as the other reference fields on the settings screen (Selection IDs, Category IDs, Custom class collection ID): a "Find your feature IDs and codes at …" link to the configured instance's `/base/classes/classes`.
...
* **Custom class collection ID is now fully optional, at save time and at sync time.** It is no longer marked required or blocked from saving when empty. `Sync_Service::run_sync()`, the single-product resync, and the grouped-product resync no longer hard-fail when it is missing either: `include_custom_classes`/`include_trade_item_custom_classes` are requested regardless, and `include_custom_collection_id` is only added to the request when a collection ID is actually configured — so custom classes are fetched across every collection instead of the sync refusing to run. The now-dead "field mapping stays inactive without a collection ID" warning path was removed along with it, since a field mapping now resolves the same way.
...
* **The one-shot admin re-syncs freeze their settings too (code review).** "Re-sync this product" and "re-sync this group" are not resumable runs, but they still write many products from one settings snapshot — and then re-read the live option for the Context ID and the stock mapping on every one of them. A save landing mid-operation could therefore split a single variable product's children across two mappings. Both paths now inject their snapshot into the upserter for the operation's duration and release it afterwards.
...
### Added — sync diagnostics

* **A warning when a queued sync never starts running.** `begin_run()` logs "Sync started" and hands off to an Action Scheduler step, but if that step is never picked up — WP-Cron not firing, or the site's own loopback requests blocked — nothing previously told the admin. A run that has sat queued for more than five minutes with no heartbeat now shows a warning in the shared banner slot on every tab of the plugin's own admin pages (Dashboard, History, Settings, Debug), reachable from a manual "Sync Now" click as much as a scheduled run. If `DISABLE_WP_CRON` is set, the message names it explicitly. This is a different failure than the existing `MAX_STALL` poison-loop guard: that one recovers a run whose steps *are* executing but making no progress; a run stuck before its first step never reaches that guard at all.
* **An on-demand health check on the Debug tab.** Reuses WordPress core's own `WP_Site_Health` tests for scheduled events and loopback requests — the same checks Tools → Site Health runs — surfaced where an admin is already looking because a sync isn't working, instead of sending them to go find Site Health themselves. Runs only on an explicit click; the loopback test performs one live HTTP request to the site's own `wp-cron.php`.
* **A "Sync not working? Check this first" checklist on the Debug tab.** Points at the health check above, WooCommerce's own logs, the Scheduled Actions screen (filtered to `skwirrel`), Site Health, `DISABLE_WP_CRON`, and loopback blocking — the same troubleshooting order support would otherwise have to explain by hand each time.

### Notes

* **`Tested up to` raised from 7.0 to 7.1.** Checked against the WordPress 7.1 field guide's full dev-note list: the iframed-editor completion only affects plugins injecting JS/CSS across the editor iframe boundary, and this plugin's product meta box is a plain server-rendered classic meta box outside that boundary; the media REST API changes (sideload validation, encode quality) apply to the `/wp/v2/media` endpoint, and this plugin's media importer calls `wp_insert_attachment()` directly, never that endpoint; the jQuery UI 1.14.2 bump does not apply, as none of this plugin's admin JS uses jQuery. No other dev note (Abilities API, `notify_post_author`, theme.json, XML-RPC, privacy-request cron, comments) touches code this plugin runs. No changes required.
