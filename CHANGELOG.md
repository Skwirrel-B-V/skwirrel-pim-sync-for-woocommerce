...
* **Stock quantity from a custom-class feature (story 6.1).** When the mapping is configured and a product carries a numeric value for it, the product is saved with `manage_stock` on and that quantity. Trade-item level features are deliberately not read: stock is a property of the product, and one product's trade items would otherwise fight over it.
...
* **Price-on-request still wins over any mapped quantity (story 6.2).** A price-on-request variation clears stock management and keeps its explicit out-of-stock status, so WooCommerce cannot re-derive availability from a quantity left over from a previous run.
...
* **A rejected Context ID is visible again (code review).** The sanitiser deliberately keeps an invalid value verbatim so it can be corrected, but the field was a `type="number"` input, whose value-sanitization algorithm reports non-numeric text as the empty string — the administrator got a blank box next to an error about a value they could no longer see. It is now a text control with numeric input hints.
