			...
			btn.addEventListener('click', function() {
				...
				.then(function(response) {
					...
					if (!response.success || !response.data.variations.length) {
						container.innerHTML = '<p class="description"><?php echo esc_js( __( 'No variation API responses found.', 'skwirrel-pim-sync' ) ); ?></p>';
						return;
					}
				...
				.catch(function() {
					spinner.classList.remove('is-active');
					btn.disabled = false;
					container.innerHTML = '<div class="notice notice-error inline"><p><?php echo esc_js( __( 'Failed to load variation responses.', 'skwirrel-pim-sync' ) ); ?></p></div>';
				});
			});
		})();
		</script>
